function! s:OverwriteColorScheme()
"  highlight! ColorColumn ctermfg=Magenta guifg=black ctermbg=236 guibg=orange
"  highlight! Conceal None
"  highlight! Todo guibg=Red guifg=Yellow ctermbg=Red ctermfg=Yellow
endfunction
...
function! s:NextColor(how, echo_color)
  ...
  for i in range(len(s:mycolors))
    ...
    try
      ...
      " overwriting default colorscheme - assert it is last loaded file
      call s:OverwriteColorScheme()
      break
    catch /E185:/
  ...
  if (a:echo_color)
    echo '[' . (s:current + 1) . '/' . len(s:mycolors) . '] ' . g:colors_name
  endif
endfunction
...
function! SetColor()
  let s:current = input("Enter number [1," . len(s:mycolors) . "]: ") - 1
  try
    execute 'colorscheme '.s:mycolors[s:current]
    " overwriting default colorscheme - assert it is last loaded file
    call s:OverwriteColorScheme()
...
function! g:SynStack()
  ...
  let g:syntaxColorElement = (map(synstack(line('.'), col('.')), 'synIDattr(v:val, "name")'))
  echo map(synstack(line('.'), col('.')), 'synIDattr(v:val, "name")')
endfunc

function! g:DumpSynStack()
  for el in g:syntaxColorElement
    let cololine = "highlight " . el . " term=None cterm=None ctermfg=None ctermbg=None gui=None guifg=None guibg=None"
    call append(line('.'), cololine)
  endfor
endfunc
